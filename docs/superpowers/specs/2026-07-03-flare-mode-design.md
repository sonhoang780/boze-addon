...
- Cost is real, but reduced upfront rather than left at the reference's value: **outer raymarch loop set to 10 iterations (down from Xor's reference 50)**, keeping the inner turbulence loop at the reference's 5 octaves (10 × 5 = 50 total noise evals vs. the reference's 250 — a 5x cut before any in-game measurement). This trades some depth/smoothness in the flame silhouette (fewer raymarch steps can look coarser/more banded, especially at the cone's edges) for a real perf headroom margin going into the first test. It's pure ALU (trig + noise math, no texture reads), unlike `TungTungSahur`'s texture-fetch-heavy SDF lookups. Cost also scales with how many screen pixels the effect actually covers (the silhouette's bounding box), not the whole screen. **Both counts stay tunable** — if 10 outer steps looks visibly too coarse once running, raise it (e.g. to 16-20) rather than accepting banding; if perf is still tight even at 10, cut the inner turbulence loop next (it's the bigger multiplier).
