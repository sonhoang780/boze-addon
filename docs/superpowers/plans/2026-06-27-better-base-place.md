# BetterBasePlace Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a BetterBasePlace AddonModule that auto-places obsidian around the nearest enemy to set up a crystal PvP base.

**Architecture:** Single module file. EventTick.Pre ms-gated loop: find target → read AC MinDamage → scan positions → calculate crystal damage → silent/alt swap obsidian → PlaceHelper.place. Registered in ExampleAddon.java.

**Tech Stack:** MC 26.1.2 Mojmap, Boze API 3.2.2, PlaceHelper, InvHelper, SwapType.Silent/Alt, ModuleManager

## Global Constraints

- MC 26.1.2 Mojmap — use `InteractionHand` not `Hand`, `net.minecraft.world.phys.BlockHitResult` not `net.minecraft.util.hit.BlockHitResult`
- Boze API 3.2.2 — `PlaceHelper`, `InvHelper`, `SwapType`, `ModeOption`, `SliderOption`, `ToggleOption` from `dev.boze.api.*`
- `InteractionMode` from `dev.boze.api.utility.interaction.InteractionMode` (NCP / Grim)
...
| File | Change |
|------|--------|
| `src/main/java/com/example/addon/modules/BetterBasePlace.java` | Create — full module |
| `src/main/java/com/example/addon/ExampleAddon.java` | Add import + `modules.add(BetterBasePlace.INSTANCE)` |

---

### Task 1: Create BetterBasePlace module

**Files:**
- Create: `src/main/java/com/example/addon/modules/BetterBasePlace.java`

**Interfaces:**
- Produces: `BetterBasePlace.INSTANCE` (static singleton) — consumed by ExampleAddon registration
    ...
    private double readACMinDamage() {
        try {
            ...
            for (Option<?> opt : ac.getOptions()) {
                if ("MinDamage".equals(opt.getName()) && opt.getValue() instanceof Number n) {
...
```
git add src/main/java/com/example/addon/modules/BetterBasePlace.java
git commit -m "feat: add BetterBasePlace module (obsidian base placement for crystal PvP)"
```

---

### Task 2: Register in ExampleAddon

**Files:**
- Modify: `src/main/java/com/example/addon/ExampleAddon.java`

**Interfaces:**
- Consumes: `BetterBasePlace.INSTANCE` from Task 1

- [ ] **Step 1: Add import + registration**

Add after the `MaceAura` import line:
```java
import com.example.addon.modules.BetterBasePlace;
```

Add after `modules.add(MaceAura.INSTANCE);`:
```java
...
git commit -m "feat: register BetterBasePlace in ExampleAddon"
```
