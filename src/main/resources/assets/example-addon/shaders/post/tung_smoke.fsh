...
void main() {
    ...
    float bodyYawDeg = decodeFloat24(getVal(17), -180.0, 180.0);

    // The visible mesh is rotated by Axis.YP.rotationDegrees(180 - renderYaw) when
    // drawn (see TungTungSahur.onWorldRender). The SDF atlas is baked in the mesh's
    // OWN local space, so localPos must be rotated by the INVERSE of that same
    // rotation before sampling it, or the raymarch samples the wrong slice of the
    // atlas for any yaw other than 0 (i.e. almost always, since the companion
    // continuously turns to follow the player) -- this was why no smoke ever
    // rendered despite fadeAlpha/geometry all being correct.
    float invYawRad = radians(-(180.0 - bodyYawDeg));
    float cy = cos(invYawRad);
    float sy = sin(invYawRad);
    mat2 invYawRot = mat2(cy, -sy, sy, cy);
    ...
    for(int i = 0; i < MAX_STEPS; i++) {
        ...
        localPos.xz = invYawRot * localPos.xz;
