...
public class GifGUI extends AddonModule {
    ...
    // ── FILTER: Tên class chứa các screen hợp lệ ──
    // Boze ClickGUI thường có "ClickGUI" hoặc "Gui" trong class name
    // TitleScreen là màn hình chính
    private static boolean isAllowedScreen(net.minecraft.client.gui.screen.Screen screen) {
        if (screen == null) return false;
        if (screen instanceof TitleScreen) return true;
        String name = screen.getClass().getName().toLowerCase();
        // Boze GUI classes thường chứa "clickgui", "gui", "menu", "boze"
        return name.contains("clickgui") || name.contains("boze")
    ...
    private GifGUI() {
        ...
        // Hook vào mọi screen, nhưng chỉ render khi đúng loại screen
        ScreenEvents.BEFORE_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ScreenEvents.beforeRender(screen).register((screenObj, context, mouseX, mouseY, tickDelta) -> {
                if (this.active && isAllowedScreen(screenObj)) {
    ...
    private void renderBackground(DrawContext context, int screenW, int screenH) {
        if (isLoading || gifFrames.isEmpty()) return;
        ...
        // Vẽ GIF full màn hình (nền của toàn bộ GUI)
